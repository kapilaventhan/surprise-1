  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, isEdit = false) => {
    ...
    if (file) {
      ...
      if (isEdit && editingPhoto) {
        setEditingPhoto({ ...editingPhoto, url: path });
      } else {
        setNewPhotoUrl(path);
      }
    }
              ...
              <div className="glass-panel rounded-3xl p-6 sm:p-8 max-w-md w-full border border-[#FF6FAE]/50 space-y-4 relative">
                ...
                <form onSubmit={handleAddSubmit} className="space-y-4 text-left">
                  <div>
                    ...
                    <p className="text-[10px] text-pink-200/80 mb-2 leading-tight">
                      Note: Since there is no backend storage, runtime uploads cannot be permanently shared across devices. 
                      Please manually place the selected image in the <code className="text-pink-300">public/images/</code> folder before deploying.
                    </p>
                    <input
                      type="file"
                      accept="image/*"
